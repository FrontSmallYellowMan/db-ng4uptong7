...
import 'rxjs/Rx';
@Component({
  selector: 'return-financeinfo',
  templateUrl: './financeinfo.component.html',
  styleUrls: ['./financeinfo.component.scss']
})

export class returnFinanceinfoComponent implements OnInit {
  @Input() nameType;
  @Input() billTypeId;
  @Input() returnMoney;
  ...
  @Output() ReBackTableData = new EventEmitter();
  @Output() backInvoice = new EventEmitter();
  @Output() returnMaterialData = new EventEmitter();
  ...
  public removeItems(tabledata, item) {
    ...
    for (let i = 0, len = this.meterialData.length; i < len; i++) {
      if (this.meterialData[i].internalinvoiceno == item.internalinvoiceno) {
        this.meterialData.splice(i, 1);
      }
    }
          ...
          if (modalDate.length == 1) {
            ...
            this.dbhttp.post("InvoiceRevise/GetMaterial", this.modalSendArray, options
              ...
              .then(res => {
                ...
                console.log(this.backTableDate);
                //保存数据
...
export class testTable {
  ...
  originalbackmoney: "0"
  originalbatchno: ""
  originaldescription: "NETBACKUP ENT SERVER软件及服务"
  originalmaterialcode:"000000000038312862"
  originalmoney:"2"
  originalstoragelocation:"0601"
  projcode:"000010"
}
