...
export class PageNo { }
@Component({
    templateUrl: 'return-examine.component.html'
    // styleUrls: ['bill-app.component.scss']
})
export class returnBillExamineComponent implements OnInit {
    ...
    public addressData(e) {
        if (this.billBack.invoice) {
            for (let i = 0, len = this.billBack.invoice.length; i < len; i++) {
                console.log(this.billBack.invoice[i].internalinvoiceno, this.invoicenoActive);
    ...
    public gettransittype(e) {
        let data = e.text;
        this.applyDate.transittype = data
    }
    //各种验证
    ...
    public NumFloatAlert(table, item) {
        ...
        console.log(this.BillmoneyWarn)
    }
    //验证两位小数
    ...
    public defaulthomepickup;
    public defaulttransittype;
    public defaultTransitMode = [];
    public accessoryExamine;//附件信息
    public materialExamine;//物料表单信息
    public returnType;//选中小类
    public examineApply;//
    ...
    public getPostMaterial(e, item) {
        ...
        this.http.post(url, [], options)
            ...
            .then(res => {
                ...
                console.log(item.descript, localData);
            })

    }
    public invoicenoActive;//当前系统发票号
    ...
    ngOnInit() {
        ...
        let headers = new Headers({ 'ticket': localStorage.getItem('ticket') });
        let options = new RequestOptions({ headers: headers });
        let url = "InvoiceRevise/GetApplyById/" + data
        this.http.post(url, {})
            .toPromise()
            .then(res => {
                ...
                // if(this.applyDate.homepickup == 0){
                //     JSON.stringify(this.applyDate.homepickup) = "是"
                // }else if(this.applyDate.revisetypecode == "1"){
                //     this.applyDate.revisetypecode = "否"
                // }
                //基础信息默认选项
                this.defaultList = new defaultdata(
                    ...
                    [{ id: this.applyDate.revisetypecode, text: this.applyDate.homepickup }],
                    [{ id: this.applyDate.revisetypecode, text: this.applyDate.transittype }]
                )
                ...
                this.defaulthomepickup = this.defaultList.homepickupList;
                if (this.defaulthomepickup[0].text == 0) {
                    this.defaulthomepickup[0].text = '是';
                }
                if (this.defaulthomepickup[0].text == 1) {
                    this.defaulthomepickup[0].text = '否';
                }
                this.defaulttransittype = this.defaultList.transittypeList;
                //财务信息
                ...
                this.invoicenoActive = this.materialExamine[0].internalinvoiceno;
                this.materialList = [];
                for (let i = 0, len = this.materialExamine.length; i < len; i++) {
                ...
                for (let i = 0, len = this.examineMoney.length; i < len; i++) {
                    ...
                    if (this.examineMoney[i].internalinvoiceno == this.invoicenoActive) {
                        this.defaultTransitMode.push(
                            {
                                id: this.examineMoney[i].TransitModeCode,
                                text: this.examineMoney[i].TransitMode
                            }
                        )
                    }
                }
                ...
                else if (this.examineApply.revisetype == "换货") {
                    this.returnType = "swapGoods";
                }
    ...
    public selecteRedtypes(value) {
        ...
        console.log(this.returnType, value);
...
export class defaultdata {
    constructor(
        ...
        public homepickupList,
        public transittypeList
    ) { }
}
