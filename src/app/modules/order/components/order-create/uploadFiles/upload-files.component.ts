...
export class UploadFilesComponent implements OnInit {
  ...
  @Input() isUpload: boolean = true;//是否要上传
  @Input() isDelete: boolean;//是否要删除
  ...
  ngOnInit() {
    ...
    this.uploader.onAfterAddingAll = (data => {
      ...
      if ((this.uploader.queue.length + this.accessoriesInfo.length) > this.maxFileNum) {
        this.uploader.queue.length = this.maxFileNum - this.accessoriesInfo.length;
  ...
  uploadFile(item: FileItem) {
    ...
    item.onSuccess = (response, status, headers) => {
      ...
      let oldPath = $("#uploadFilesNode").val();
      let clearPath = false;
      ...
      if (clearPath) {
        this.clearPath = true;
        this.filePath = $("#uploadFilesNode").val();
        $("#uploadFilesNode").select();//选中文件路径部分
        document.execCommand('Delete');//HTML文档切换到设计模式(designMode)时，文档对象暴露 execCommand方法,Delete为方法固定名称。删除路径
        $("#uploadFilesNode").blur();//失去焦点后，ie自动给上传队列中添加该文件，故在添加文件处将最后一个文件删掉
      }
    }
  ...
  //下载已上传文件
  loadAccessory(url) {
    window.open(this.orderCreateService.upFilesDownload(url));
  }
  ...
  removeFile(item: FileItem, i: number) {
    ...
    if (this.uploader.queue && (this.uploader.queue.length + this.accessoriesInfo.length) < this.maxFileNum) {
      this.isUpload = true;
    }
