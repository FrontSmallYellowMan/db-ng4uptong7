...
export class RegisterBarCode {
    ...
    extendBarCodeTools() {
            ...
            return $(jQ).each(function (item, index) {
                ...
                switch (settings.code) {
                    case 'code39':
                        ...
                        break;
                    default:
                        window.alert("Code" + settings.code + ' not implemented');
                        return '';
                };
                var img = $("<img />").width("350px").height("100%").attr("src", $.barcode._createBmp([encoded], $.barcode.palette));
        ...
        $.barcode._createBmp = function (grid, palette) {
            ...
            var bitmap = bitmapFileHeader + bitmapInfoHeader + rgbQuadStr + data.join('');
            bitmap = bitmap.replace(/yyyy/, this._multiByteEncode(bitmapFileHeader.length + bitmapInfoHeader.length + rgbQuadStr.length, 4));
