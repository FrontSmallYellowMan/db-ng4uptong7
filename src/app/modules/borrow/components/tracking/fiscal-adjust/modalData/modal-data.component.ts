...
import { FiscalAdjustService } from './../../../../services/fiscal-adjust.service';
...
export class FiscalModalDataComponent implements OnInit {
  modal: XcModalRef;
  modalData = {
    checkDataList: [],
    headDataList: [],
  ...
  identical(modalData) {//验证overallIdentical
    console.log(JSON.stringify(modalData.identical));
  ...
  saveModalData() {//保存数据
    ...
    this.fiscalAdjustService.batchImportFiscal().then(callback);//导入数据
  }
}