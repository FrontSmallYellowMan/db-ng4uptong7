...
@Component({
  selector: 'db-huaweiownservice',
  templateUrl: './huaweiownservice.component.html',
  styleUrls: ['./huaweiownservice.component.scss','../../scss/ec-template.component.scss']
})
export class HuaweiownserviceComponent implements OnInit {
  ...
  getECPackageResult(){
    ...
    this.templateService.getECPackageResult(this.localUserInfo["YWFWDM"]).subscribe(data => {
      if (data.Result) {
        ...
        this.selectDataSource["PaymentType"] = this.ecComTools.PaymentServiceHuaWeiOwn;
  ...
  /** 按钮组功能 */
    ...
    formValid(){
      ...
      if (this.formData.BaseData.PaymentType == this.selectDataSource.PaymentType[0]['id']) {
        if (!this.formData.BaseData.PayType_Ticket_Day || !this.formData.BaseData.PayType_Ticket_TypeID) {
      ...
      if (this.formData.BaseData.PaymentType == this.selectDataSource.PaymentType[2]['id']) {
        if (!this.formData.BaseData.PayType_OP_TicketID || !this.formData.BaseData.PayType_OP_Day) {
          this.windowService.alert({ message: "请维护付款条款信息", type: "fail" });
          return false;
        }
      }
