...
@Component({
  selector: 'db-huaweiserviceresale',
  templateUrl: './huaweiserviceresale.component.html',
  styleUrls: ['./huaweiserviceresale.component.scss','../../scss/ec-template.component.scss']
})
export class HuaweiserviceresaleComponent implements OnInit {
  ...
  getECPackageResult(){
    ...
    this.templateService.getECPackageResult(this.localUserInfo["YWFWDM"]).subscribe(data => {
      if (data.Result) {
        ...
        this.selectDataSource["PaymentType"] = this.ecComTools.PaymentServiceHuaWei;
    ...
    if (item && item.length > 0) {
      switch (this.formData.BaseData.PaymentType) {
        case this.selectDataSource.PaymentType[0]['id']:
          ...
          break;
        case this.selectDataSource.PaymentType[2]['id']:
          this.formData.BaseData.PayType_OP_TicketName = item[0]["TT_Name"];
          break;
        case this.selectDataSource.PaymentType[4]['id']:
  ...
  /** 付款条款 （3）日期选择 */
  onOPBillTimeChange(){
    this.formData.BaseData.Payment_BillDate = moment(this.formData.BaseData.Payment_BillDate).format("YYYY-MM-DD");
  }
  /** 付款条款 （3） 百分比 blur事件*/
  onBlurOPRatio(){
  ...
  /** 付款条款（4） 百分比 blur事件*/
  onBlurOP2Ratio(){
    if (this.formData.BaseData.PayType_OP2_Ratio) {
      if(this.formData.BaseData.TotalMoney != undefined && this.formData.BaseData.TotalMoney.toString() != ""){
        //计算金额 TotalMoney * PayType_OP_Ratio  保留两位小数 四舍五入
        let caleResult = this.formData.BaseData.TotalMoney * (parseFloat(this.formData.BaseData.PayType_OP2_Ratio) / 100);
        this.formData.BaseData.PayType_OP2_Money = caleResult.toFixed(2);
      }else{
  ...
  /** 按钮组功能 */
    ...
    formValid(){
      ...
      if (!this.formData.BaseData.SellerERPCode) {
        this.windowService.alert({ message: "请选择乙方", type: "fail" });
        return false;
      }
      //供应商
      if (!this.formData.BaseData.SupplierCodeSAP) {
      ...
      if (this.formData.BaseData.PaymentType == this.selectDataSource.PaymentType[0]['id']) {
        if (!this.formData.BaseData.PayType_Ticket_Day || !this.formData.BaseData.PayType_Ticket_TypeID || !this.formData.BaseData.Payment_InvoiceTaxRate) {
      ...
      if (this.formData.BaseData.PaymentType == this.selectDataSource.PaymentType[1]['id']) {
        if (!this.formData.BaseData.PayType_Transfer_Day || !this.formData.BaseData.Payment_InvoiceTaxRate) {
          this.windowService.alert({ message: "请维护付款条款信息", type: "fail" });
          return false;
        }
      }
      ...
      if (this.formData.BaseData.PaymentType == this.selectDataSource.PaymentType[2]['id']) {
        if (!this.formData.BaseData["Payment_BillDate"] || !this.formData.BaseData.PayType_OP_TicketID || !this.formData.BaseData.Payment_InvoiceTaxRate) {
          this.windowService.alert({ message: "请维护付款条款信息", type: "fail" });
          return false;
        }
      }
      ...
      if (this.formData.BaseData.PaymentType == this.selectDataSource.PaymentType[3]['id']) {
        if (!this.formData.BaseData.PayType_OP_SigDay || !this.formData.BaseData.Payment_InvoiceTaxRate) {
          this.windowService.alert({ message: "请维护付款条款信息", type: "fail" });
          return false;
        }
      }
      ...
      if (this.formData.BaseData.PaymentType == this.selectDataSource.PaymentType[4]['id']) {
        if (!this.formData.BaseData.PayType_PP_Day || !this.formData.BaseData.PayType_PP_TicketID) {
          this.windowService.alert({ message: "请维护付款条款信息", type: "fail" });
          return false;
        }
      }
      ...
      if (this.formData.BaseData.PaymentType == this.selectDataSource.PaymentType[5]['id']) {
        if (!this.formData.BaseData.PayType_Full_Day || !this.formData.BaseData.Payment_InvoiceTaxRate) {
          this.windowService.alert({ message: "请维护付款条款信息", type: "fail" });
          return false;
        }
      }
      //城市区县
      if (!this.formData.BaseData.SignedCityCode || !this.formData.BaseData.SignedCounty) {
        this.windowService.alert({ message: "请选择城市区县", type: "fail" });
        return false;
      }
      //合同日期
      ...
      //合同附件至少上传一个
      if (this.formData.Accessories.length == 0) {
        this.windowService.alert({ message: "合同附件至少上传一个", type: "fail" });
        return false;
      }
      return true;
    }
    ...
    onSaveBefore(){
      ...
      //争议解决方式默认值
      this.formData.BaseData.DisputeDealtType = "0";
    }
    /**保存 */
    onSave(type){
      this.loading = true;
