...
@Component({
  selector: "select-search-city",
  templateUrl: './select-search-city.component.html',
  styleUrls: ['./select-search-city.component.scss'],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => SelectSearchComponentCity),
      multi: true
    }
  ]
})
export class SelectSearchComponentCity implements ControlValueAccessor {
  constructor() {}
  ...
  search(keyWord: string){
    ...
    this._optionList = this.optionList.filter(item => {
      return item["CityCode"].indexOf(keyWord) >= 0 || item["CityName"].indexOf(keyWord) >= 0;
    });
  }
