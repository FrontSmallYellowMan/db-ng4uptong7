export class PurchaseOrderObj {//采购订单-整体数据结构
  ...
  /*
    PurchaseRequisitionType:表示创建NB单的类型来源
    所有类型：
        a.正常创建-选择了若干采购申请
        b.正常创建-直接创建NB
        c.已完成-合同采购申请-提交采购订单
  ...
  */
  ...
  public OrderType: string = "NB";//采购订单类型（NB）
...
export class PurchaseOrderDetails {//采购清单-物料信息
  ...
  public New_Sc_Code:any;//新的销售合同号
  public SCType:any;//判断合同是解除或者变更（2：变更，3：解除）
}
...
/*————————————————————————————————————————————— 以下是需要的其他结构 ————————————————————————————————————————————*/
export class NBOrderMessage {//NB单采购订单-采购信息-数据结构
  public TemplateID = null;//模板id
  public CompanyName: string = "";//公司名称
  public CompanyCode: string = "";//公司代码
  public FactoryCode: string = "";//工厂编号
...
export class NBNewPersonElement {//预审人员的 单位信息 (在暂存情况下)
