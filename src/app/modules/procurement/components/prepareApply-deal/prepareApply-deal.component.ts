...
import { PrepareApply } from '../../services/prepareApply-submit.service';
...
// part7-审批人-end
// part8-整体-start
    prepareApplyData=new PrepareApply();//整体数据
    ISRMB=true;//是否 人民币 标识
    isView: boolean = true;//是否查看页面 查看页面(true) 审批页面(false)
    hasContract;//表示 预下采购申请的方式：有合同-true；无合同-false
    submiting=false;//审批中
// part8-整体-end    
    constructor(
    ...
    ngOnInit() {
        this.prepareApplyData.PurchaseContractInfo=new PurchaseContractInfo();
        //初始 获取参数
...
// part3-采购清单-start
    showOrder() {//预览采购清单
        let modalData = {
            procurementList:this.prepareApplyData.PurchaseRequisitionDetailsList,
            untaxAmount: this.procurementListPrice.excludetaxmoney,
            factory: this.prepareApplyData.factory,
            vendor: this.prepareApplyData.vendor
        }
...
// part7-审批-start
    ...
    approvalCompleteDel(data){//审批后的处理
        if (data.Result) {
            ...
            if(this.approvalType=="agree" && this.prepareApplyData.istoerp &&
                this.getFinalNodeId()==this.activeNode){//最后节点的同意 直接跳转
                this.router.navigate(['procurement/deal-prepareapply/'+ this.urlParamObj.applyid]);
            }else{
                setTimeout(()=>{
                    window.close();
                },3000);
...
// part8-整体-start
    getProcurementData(id) {//获取整单数据
        ...
        this.dbHttp.get(url).subscribe(data => {
            if (data.Result) {
                this.prepareApplyData = JSON.parse(data.Data);
                //保存自定义表单项
                this.prepareApplyData.PurchaseProjectInfo.ProjectInfo?this.prepareApplyData.PurchaseProjectInfo.ProjectInfo=JSON.parse(this.prepareApplyData.PurchaseProjectInfo.ProjectInfo):'';
                //将详情中获取的自定义表单项重构
                this.getBussinessFieldConfigCallBack(this.prepareApplyData.PurchaseProjectInfo.ProjectInfo);
                console.log("整单数据");
                console.log(this.prepareApplyData);
                // #0-整体
                if(this.prepareApplyData.purchaserequisitiontype=="预下单采购_无合同"){//无合同
                ...
                //保存发货方式
                if(this.prepareApplyData.SendType){
                    this.SendType=JSON.parse(this.prepareApplyData.SendType);
                }

                if (this.prepareApplyData.TemplateID) {//获取模板名称
                    this.procurementTemplateService.getProcurementTplOne(this.prepareApplyData.TemplateID).then(data => {
                        if (data.Result) {
                            this.TemplateName = data.Data&&data.Data.Name?data.Data.Name:'';
                        }
                    })
                }
                let localvendorno = "";
                localvendorno = JSON.stringify(Number(this.prepareApplyData.vendorno));
                if (localvendorno.substring(0, 2) == "10") {//判断是否10开头
                    this.isTenStart = true;
                } else {
                    this.isTenStart = false;
                }
                if (this.prepareApplyData.currencycode == "RMB") {//判断是否人民币
                    this.ISRMB = true;
                }else{
                    this.ISRMB = false;
                }
                if(this.prepareApplyData.VendorCountry==1){//国外
                    let deliveryAllInfo = {
                        condition:[],//交货条件
                        people:[]//收货人
                    }
                    this.shareDataService.getDeliverySelectInfo().then(data => {//交货条件和收货人 数据
                        ...
                        for(let i=0,len=deliveryAllInfo.condition.length;i<len;i++){//国际贸易条件
                            if(deliveryAllInfo.condition[i]["id"]==this.prepareApplyData.internationaltradeterms){
                                this.abroadData.conditionStr=deliveryAllInfo.condition[i]["text"];
                            }
                        }
                        for(let i=0,len=deliveryAllInfo.people.length;i<len;i++){//收货人
                            if(deliveryAllInfo.people[i]["id"]==this.prepareApplyData.receiver){
                ...
                // #3-采购清单
                this.prepareApplyData.PurchaseRequisitionDetailsList.forEach(item => {
                    if(this.hasContract){
                ...
                if(this.prepareApplyData.PurchaseRequisitionDetailsList && this.prepareApplyData.PurchaseRequisitionDetailsList.length){
                    //有清单的时候
                    this.procurementListPrice.foreigncurrencymoney=this.prepareApplyData.foreigncurrencymoney;
                    this.procurementListPrice.excludetaxmoney=this.prepareApplyData.excludetaxmoney;
                    this.procurementListPrice.taxinclusivemoney=this.prepareApplyData.taxinclusivemoney;
                    setTimeout(()=>{
                        if(this.prepareApplyData.PurchaseRequisitionDetailsList.length >= 10){//出现滚动条的宽度调整
                            if(this.hasContract){
                                $(".w140").addClass("w146");
                            }else{
                                $(".w60").addClass("w66");
                ...
                // #4-销售信息
                // #5-支持文件&用印文件
                let i; let len = this.prepareApplyData.AccessoryList.length;
                for (i = 0; i < len; i++) {//去除附件数组中的空值
                    if (!this.prepareApplyData.AccessoryList[i]) {
                        this.prepareApplyData.AccessoryList.splice(i, 1);
                        len--;
                        i--;
                    }
                }
                this.prepareApplyData.AccessoryList.forEach(item => {//分离 支持文件和用印文件
                ...
                this.contractInfoAccessories=this.prepareApplyData.PurchaseContractInfo&&this.prepareApplyData.PurchaseContractInfo["PurchaseContractInfoAccessories"]?this.prepareApplyData.PurchaseContractInfo["PurchaseContractInfoAccessories"]:[];
                if(this.prepareApplyData.PurchaseContractInfo&&this.prepareApplyData.PurchaseContractInfo["SealInfoJson"]){
                    this.sealInfo=JSON.parse(this.prepareApplyData.PurchaseContractInfo["SealInfoJson"]);
                }
                this.printUseData=this.SubmitMessageService.removeRepeatApprovePrintData(this.prepareApplyData.PurchaseContractInfo&&this.prepareApplyData.PurchaseContractInfo.UserSetting?this.prepareApplyData.PurchaseContractInfo.UserSetting:'');
                // #6-修改记录
                this.getModifyUrl = "PurchaseManage/GetRequisitionRecord/"+this.prepareApplyData.requisitionnum;
                this.dbHttp.get(this.getModifyUrl + "/" + 1 + "/" + 10).subscribe(data => {//获取记录
                    if (data.Result) {
                        this.modifyRecord = data.Data.List;

                        //验证是否修改了我方主体，用以显示视图中的样式
                        if(this.modifyRecord.some(item=>item.Own.substring(item.Own.indexOf('-')!=-1?item.Own.indexOf('-')+1:0)!=this.prepareApplyData.company)){
                        ...
                        if(this.modifyRecord.some(item=>item.Vendor!=this.prepareApplyData.vendor)){
                        ...
                        if(this.modifyRecord.some(item=>item.taxinclusivemoney!=this.prepareApplyData.taxinclusivemoney)){
...
// part9-其他-start
    matchContractName(scCode){//根据合同唯一标识(SC_Code) 匹配合同名称 (显示清单时)
        let len=this.prepareApplyData.PurchaseRequisitionSaleContractList.length;
        let i;let item;
        for(i=0;i<len;i++){
            item=this.prepareApplyData.PurchaseRequisitionSaleContractList[i];
    ...
    if(data.length > 0){
      data.forEach((item, i) => {
        ...
        if (item.FieldShowType == "select" || item.FieldShowType == "radio") {
          ...
          tempArray.length>0?data[i].FieldValue = tempArray[0].text:data[i].FieldValue='';
        }
      });
     }
  }
