...
import { PurchaseOrderObj, NANewPersonElement } from './../../services/NA-submit.service';
...
@Component({
    templateUrl: 'NA-submit.component.html',
    styleUrls: ['NA-submit.component.scss', './../../scss/procurement.scss']
})
export class NASubmitComponent implements OnInit {
    // part1-基础信息-start
    ...
    ngOnInit() {
        ...
        if(recordId) {
            ...
            this.recordAllowEditStateQuery.NotAllowEditLink = `/procurement/deal-NA/${recordId}`;
        ...
        for (let i = 0; i < 4; i++) {//先默认让整体流程信息 至少长度为4
            this.wholeApprovalData.push(new NANewPersonElement());//前4个预审人员信息
        }
        ...
        if (this.saveData.ID) {
            ...
            let type = window.localStorage.getItem("createNAType");
            switch (type) {//判断NAType
                case "hasApply":
            ...
            if (this.NAType == "hasApply" || this.NAType == "prepareApplyNoContract") {//获取采购申请信息
        ...
        //获取销售合同的状态（变更或解除）
        this.naCommunicateService.nbChangeSaleContractGet().subscribe(data=>{
            if(data){
    ...
    // part2-采购信息-start
    onNAOrderFormValidChange(e) {//当 采购信息表单校验 变化
        this.NAOrderMessageValid = e;
    }
    onCompanyCodeChange(e) {//当 公司代码(我方主体) 变化
        this.getFlow(this.saveData.CompanyCode);
    }
    onNAOrderMessageChange(e) {//当 采购信息数据 变化
    ...
    massageValid() {//验证采购信息
        ...
        alertFun(this.saveData.DeliveryLocation, "国际贸易地点");
        alertFun(this.saveData.DeliveryCondition, "国际贸易条件");
    ...
    purchaseFormAccurateValid() {//采购清单精确进行校验
        let self = this;
        let invalidStatus: boolean = false;//采购清单列表是否合法
        let alertFun = function (val, str) {
            if (str == '库存地') {//库存地
                if (val && val.length != 4) {
        ...
        this.saveData.PurchaseOrderDetails.forEach((item, index) => {
            ...
            if (this.NAType == "hasApply" || this.NAType == "prepareApplyNoContract") {
    ...
    getApplyListLocal() {//获取采购申请信息
        ...
        if (this.saveData.ID && this.saveData.PurchaseOrderDetails && this.saveData.PurchaseOrderDetails.length) {//编辑
            ...
            this.localApplyList = JSON.parse(localStorage.getItem("NAApplyList"));
    ...
    getPurchaseOrder(id) {//获取采购订单数据
        ...
        this.dbHttp.get(url).subscribe(data => {
            if (data.Result) {
                ...
                console.log(localWholeData);
                this.saveData = data.Data;
                console.log("整单数据");
                console.log(this.saveData);
                switch (this.saveData.PurchaseRequisitionType) {//判断NAType
                    case -1:
                        this.NAType = "directNA";
                        break;
                    case 0:
                ...
                if (this.NAType == "hasApply" || this.NAType == "prepareApplyNoContract") {
                    ...
                    this.applyListLocalShow = true;
                    if (this.NAType == "hasApply") {//获取合同可采购金额额度
                        let arr = [];
                        for (let i = 0, len = this.saveData.PurchaseOrderDetails.length; i < len; i++) {
                ...
                console.log(localWholeData.ApproveID);
                // #7-审批
                if (localWholeData.ApproveID) {//驳回时获取当前审批信息
                    ...
                    this.dbHttp.get(wfidUrl).subscribe(data => {
                        ...
                        console.log(this.approvalProcessID);
                    })
                }
    ...
    saveNAOrder(type) {//提交NA
        ...
        if (!this.saveData.PruchaseAmount) {//总额必须大于0
            this.WindowService.alert({ message: '采购金额不允许为0', type: 'warn' });
            return;
        }
    ...
    viableTracenoAfterVerificate() {//需求跟踪号 可行 继续进行后面的校验
        ...
        // #4-校验合同可采购金额
        if (this.NAType == "hasApply") {//有合同情况，校验合同可采购金额
        ...
        this.dbHttp.post("PurchaseManage/SavePurchaseOrder", this.saveData, options).subscribe(data => {
            ...
            if (data.Result) {
                if (!this.saveData.ID) {//新建
                    ...
                    window.localStorage.removeItem('createNAType');
                }


                console.log(this.approvalProcessID, this.submitType);
                ...
                //     //localStorage.setItem('NA',"submit");
                //     window.close();
                // }else{
                //     //localStorage.setItem('NA',"save");
                //     window.close();
                // }
                        ...
                        this.saveData.VendorNo, this.saveData.PruchaseAmount, this.saveData.VendorTrace).then(response => {
                            if (response) {
                                if (response == "NA") {
                                    ...
                                    this.WindowService.alert({ message: "NA采购订单不允许创建其他类型的订单", type: "warn" });
                                }
                            }
                        })
                }
