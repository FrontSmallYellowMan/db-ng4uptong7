...
import { PurchaseOrderObj } from '../../services/NK-new.service';
import { ApprovalMethodService } from '../../services/approval-method.service';

@Component({
    templateUrl: 'NK-view.component.html',
    styleUrls: ['NK-view.component.scss', 
        './../../scss/procurement.scss','./../../scss/deal-page.scss']
})
export class NKViewComponent implements OnInit {
// part1-基础信息-start
// part1-基础信息-end
// part2-采购信息-start
    TemplateName;//模板名称
...
// part7-审批人-end
// part8-整体-start
    NKOrderData=new PurchaseOrderObj();//整体数据
...
// part3-采购清单-start
    showOrder() {//预览采购清单
        let modalData = {
            procurementList:this.NKOrderData.PurchaseOrderDetails,
            untaxAmount: this.NKOrderData.PruchaseAmount,
            factory: this.NKOrderData.FactoryCode,
            vendor: this.NKOrderData.Vendor
        }
    ...
    approvalCompleteDel(data){//审批后的处理
        if (data.Result) {
            ...
            if(this.approvalType=="agree" && 
                ...
                this.router.navigate(['procurement/deal-NK/'+ this.urlParamObj.applyid]);
            }else{
                setTimeout(()=>{
                    window.close();
                },3000);
...
// part8-整体-start
    getProcurementData(id) {//获取整单数据
        ...
        this.dbHttp.get(url).subscribe(data => {
            if (data.Result) {
                this.NKOrderData = data.Data;
                console.log("整单数据");
                console.log(this.NKOrderData);
                // #1-基础信息
                // #2-采购信息
                if (this.NKOrderData.TemplateID) {//获取模板名称
                    this.procurementTemplateService.getProcurementTplOne(this.NKOrderData.TemplateID).then(data => {
                        if (data.Result) {
                ...
                let localvendorno = "";
                localvendorno = JSON.stringify(Number(this.NKOrderData.VendorNo));
                if (this.NKOrderData.CurrencyCode == "RMB") {//判断是否人民币
                    this.ISRMB = true;
                }else{
                    this.ISRMB = false;
                }
                // #3-采购清单
                this.NKOrderData.PurchaseOrderDetails.forEach(item => {//采购清单的物料来源显示 进行拼接
                    this.procurementListPrice.numAmount += item.Count;//数量总计
                });
                // #4-销售信息
                // #5-支持文件&用印文件
                let i; let len = this.NKOrderData.PurchaseOrderAccessories.length;
                for (i = 0; i < len; i++) {//去除附件数组中的空值
                    if (!this.NKOrderData.PurchaseOrderAccessories[i]) {
                        this.NKOrderData.PurchaseOrderAccessories.splice(i, 1);
                        len--;
                        i--;
                    }
                }
                this.NKOrderData.PurchaseOrderAccessories.forEach(item => {
