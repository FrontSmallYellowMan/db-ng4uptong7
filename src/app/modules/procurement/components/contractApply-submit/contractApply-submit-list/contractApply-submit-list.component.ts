...
@Component({
    ...
    styleUrls: ['contractApply-submit-list.component.scss',
...
export class ContractApplySubmitListComponent implements OnInit,OnChanges {
    ...
    fileUploadApi = "api/PurchaseManage/UploadPurchaseRequisitionDetails/0";//批量上传路径
    ...
    deleteList(){//批量删除采购清单列表
        ...
        for (i = 0; i < len; i++) {
            ...
            if (item.checked === true) {
                    ...
                    this._purchaseData.procurementList = JSON.parse(JSON.stringify(this._purchaseData.procurementList));// 重新保存物料列表，用来重置循环绑定的form表单项
                    len--;
                    i--;
            }   
        }
