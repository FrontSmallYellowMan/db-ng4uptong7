...
import { SaleTabElement,PurchaseRequisitionSaleContractList,
    SellShowElement } from './../../../services/prepareApply-submit.service';

@Component({
    selector: "prepareApply-submit-sell",
    templateUrl: 'prepareApply-submit-sell.component.html',
    styleUrls: ['prepareApply-submit-sell.component.scss',
        './../../../scss/procurement.scss','./../../../scss/submit-sell.scss']
})
export class PrepareApplySubmitSellComponent implements OnInit,OnChanges {
            ...
            for(let i=0,len=this.contractList.length;i<len;i++){
                ...
                this.dbHttp.get(url).subscribe(data => {//获取合同详细信息
                    if (data.Result) {
                        ...
                        switch(this.showList[i].State){//销售合同状态
                            case 0:
                                this.showList[i].State="草稿";
                                break;
                            case 1:
                                this.showList[i].State="申请中";
                                break;
                            case 2:
                                this.showList[i].State="已完成";
                                break;
                            case 3:
                                this.showList[i].State="驳回";
                                break;
                        }
    ...
    deleteTab(e) {//删除一个 tab(合同)
        ...
        window.localStorage.setItem("prepareContractList", JSON.stringify(this.contractList));//重新存放
        setTimeout(()=>{//需要延迟
    ...
    openContractDetail(code){//查看销售合同详情页
        window.open(dbomsPath+'india/contractview?SC_Code='+code);
    }
