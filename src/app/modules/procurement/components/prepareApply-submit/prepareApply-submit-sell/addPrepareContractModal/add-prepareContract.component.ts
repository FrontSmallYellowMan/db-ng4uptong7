...
@Component({
    selector: "add-prepareContractModal",
    templateUrl: 'add-prepareContract.component.html',
    styleUrls:['add-prepareContract.component.scss',
      './../../../../scss/procurement.scss','./../../../../scss/add-contract.scss']
})
export class AddPrepareContractModalComponent implements OnInit {
  ...
  ngOnInit(){
    ...
    this.rank = new Rank();
    this.selectedList = JSON.parse(window.localStorage.getItem("prepareContractList"));//获取已选列表
  }
  clearSearch(){//重置搜索
    this.query = new Query();
    this.sale.person="";
  ...
  initData(query: Query){
    ...
    this.query.PurchaseType="YX";//合同类型
