...
import { SelectSealsModalComponent } from './selectSeals-modal.component';
...
export class AddSealsComponent implements OnInit {
  @Output() scSeals = new EventEmitter();
  @Input() sealsInfo;
  @Input() platfromid;
  @Input() isEdit:boolean;
  @Input() set editData(data){
  ...
  ngOnInit() {
      //在初始化的时候 创建模型
      this.modalAddForm = this.xcModalService.createModal(SelectSealsModalComponent);
      //模型关闭的时候 如果有改动，请求刷新
