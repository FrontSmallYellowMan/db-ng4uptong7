...
import { BuyerInfo } from "../../../services/prepareApply-submit.service";
import { ShareDataService } from "../../../services/share-data.service";

@Component({
  selector: 'customInforModal',
  templateUrl: './customInforModal.component.html',
  styleUrls: ['./customInforModal.component.scss']
})
export class CustomInforModalComponent implements OnInit {
  ...
  modalTitle: string = "买方信息";//模态窗头部标题
  ...
  getBuyerList(query) {
    if (!query) return;
    this.loading = true;
    this.shareDataService.getBuyerInfoByName(query).subscribe(
      data => {
        this.loading = false;
  ...
  getBuyerInfoByerpCode(erpCode){
    if (erpCode && erpCode != "A") {
      this.loading = true;
      this.shareDataService.getBuyerInfoByErpCode(erpCode).subscribe(
        data => {
          this.loading = false;
          if (data.Result) {
