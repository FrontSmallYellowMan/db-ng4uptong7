// NB单页面-采购清单
import { Component, OnInit, Input, EventEmitter, Output, ElementRef,
     ViewChild, AfterViewInit, DoCheck,OnChanges,SimpleChanges } from '@angular/core';
...
@Component({
    ...
    styleUrls: ['NB-new-list.component.scss', 
...
export class NBNewListComponent implements OnInit,OnChanges {
    ...
    checkedNum = 0;//已选项数
    clospanNum=12;//不同情况下 列数的不同
    tempList=[];//暂存刚开始获取的已选采购申请列表
    fileUploadApi_HasApply="api/PurchaseManage/UploadPurchaseOrderDetails/0/2";//有采购申请-批量上传路径
    fileUploadApi_DirectNB="api/PurchaseManage/UploadPurchaseOrderDetails/0/3";//直接创建NB-批量上传路径
    fileUploadApi_preNoContract="api/PurchaseManage/UploadPurchaseOrderDetails/0/1";//预下无合同-批量上传路径
    upLoadData;//上传文件接口相关参数
    _NBType;//创建NB单类型 （目前三种类型：hasApply，directNB，prepareApplyNoContract）
    ...
    ngOnChanges(changes: SimpleChanges){
        ...
        if(changes["NBType"] && changes["NBType"].currentValue){//新建 NBType传入值
            if(!this.purchaseOrderID && 
                (changes["NBType"].currentValue=="hasApply" || changes["NBType"].currentValue=="prepareApplyNoContract")){
    ...
    ngDoCheck() {
        ...
        if (JSON.stringify(this._purchaseData.procurementList) != JSON.stringify(this.beforeProcurementList) 
            && this._purchaseData.procurementList && this._purchaseData.procurementList.length) {//编辑状态下，当传入采购清单变化
                ...
                if(this._NBType=="hasApply" && this.purchaseOrderID && this.firstCreateContractList){
            ...
            if(this._NBType=="prepareApplyNoContract" && this.purchaseOrderID && !this.prepareApplyNoContract_requisitionnum){
                //存 预下无合同 采购申请单号
                this.prepareApplyNoContract_requisitionnum=this._purchaseData.procurementList[0]["PurchaseRequisitionNum"];
            }
            this.calculateTotalTax("numberChange");
        }
    }
    getApplyAlreadyExistList(){//新建时请求已有采购申请的清单
            let list = JSON.parse(window.localStorage.getItem("applyList"));
            this.tempList=list;
            if (list && list.length) {
                if(this._NBType=="prepareApplyNoContract"){//存 预下无合同 采购申请单号
                                     ...
                                     if(this._NBType=="hasApply"){
                                        ...
                                        if(ele){
                                            ...
                                            */
                                            ...
                                            this.onPurchaseDataChange.emit(this._purchaseData);
                                            //this.SaleContractCodeAndName[j]=newItem["MaterialSource"];//保存销售合同code

                                        }
                                    }
                                    if(this._NBType=="prepareApplyNoContract"){
                                        let newItem=new PurchaseOrderDetails();
                                        this.getApplyAlreadyExistItem(newItem,details[j]);

                                        newItem["MaterialSource"]=details[j]["MaterialSource"];
                                        newItem["PurchaseRequisitionNum"]=this.prepareApplyNoContract_requisitionnum;
    ...
    addProcurementItem() {//增加一项采购清单
        ...
        if (this.contractListLength == 1 && this._NBType=="hasApply") {//若合同列表只有一项,直接选入
        ...
        if(this._NBType=="prepareApplyNoContract"){//预下单采购_无合同
            this._purchaseData.procurementList[len - 1]["MaterialSource"] = "PL";
            this._purchaseData.procurementList[len - 1]["PurchaseRequisitionNum"] = this.prepareApplyNoContract_requisitionnum;
        }
    ...
    downloadTpl() {//下载采购清单模板
        switch(this._NBType){
            case "hasApply":
                window.open(dbomsPath+'assets/downloadtpl/NB新建-采购清单-有合同.xlsx' );
                break;
            case "directNB":
                window.open(dbomsPath+'assets/downloadtpl/NB新建-采购清单-直接创建.xlsx' );
                break;
            case "prepareApplyNoContract":
                window.open(dbomsPath+'assets/downloadtpl/NB新建-采购清单-预下无合同.xlsx' );
                break;
        }
    }
    ...
    uploadPurchase(e) {//批量上传
        if (e.Result) {
            ...
            result.forEach(item => {//分别匹配
                ...
                item["isImport"] = false;
                // if(this._NBType=="directNB"){
                //     item["MaterialSource"]=this.directNBMatchContract(item["MaterialSource"]);
                // }
                if(this._NBType=="hasApply"){
                ...
                if(this._NBType=="prepareApplyNoContract"){
                    item["PurchaseRequisitionNum"] = this.prepareApplyNoContract_requisitionnum;
                }
    ...
    materialSourceChange(i) {
        if (this._NBType=="hasApply" && this._purchaseData.procurementList[i].DBOMS_PurchaseRequisitionSaleContract_ID) {//已选择
        ...
        if(this._NBType=="prepareApplyNoContract" && this._purchaseData.procurementList[i].MaterialSource){//已选择
            this._purchaseData.procurementList[i].PurchaseRequisitionNum=this.prepareApplyNoContract_requisitionnum;
        }
        this.onPurchaseDataChange.emit(this._purchaseData);
    }
    ...
    directNBMatchContract(sou){//当直接创建NB时，根据物料来源匹配
        if(sou!="BH" && sou!="PL"){
            this.windowService.alert({ message: '导入列表物料来源不是BH或PL', type: 'warn' });
            this.matchContractPrompt=true;
            return '';
        }
        return sou;
    ...
    getSaleContractState(procurementList){
      if(procurementList){
          ...
          //console.log(this.SaleContractCodeAndName);
      ...
      this.shareMethodService.getSaleContractStateApi(saleContractCode).then(data=>{
          if(data.Result){
              if(data.Data.length>0){
                  ...
                  this.nbCommunicateService.nbChangeSaleContractSend(this.changeSaleContractCodeAndName);
