...
import { ApplyData, BaseDataSource, Invoice, Material, MaterialList, AccessItem, TemporaryAddress, ApproveUsers } from "../../datamodel/redapply";
...
import { environment, APIAddress, dbomsPath } from "../../../../../environments/environment";
...
@Component({
  selector: 'db-return-apply',
  templateUrl: './return-apply.component.html',
  styleUrls: ['./return-apply.component.scss']
})
export class ReturnApplyComponent implements OnInit {
  ...
  @ViewChild('redApplyForm') redApplyForm: NgForm;//表单
  DOAImportTemplateAddress = dbomsPath + "assets/downloadtpl/换货DOA类物料导入模板.xlsx";
  uploadInvoice:FileUploader;
  ...
  provincecityinfo: any = {};//省市县下拉框数据源
  ...
  tabitemi: any = 0;//记录点击了哪个tab下的添加临时地址 下标
  addressIndex = -1;//记录临时地址选中项在数据源中的下标
  temporaryAddressDataSource: TemporaryAddress[] = [];//临时地址
  temporaryAddressModalType: boolean = false;//临时地址模态窗是否编辑数据状态
  isBatchUploadForDOA: boolean = false;//财务信息 是否批量上传
  isDOA: boolean = false;//是否doa类
  loading: any = false;
  isrezty: boolean = false;//是否rezty 默认 不是rezty
  isreztychange = 1;//变化次数
  approverUsersInfo = [];//审批人信息
  ngOnInit() {
    this.uploadInvoice = new FileUploader({ url: environment.server+this.redApplyService.batchImportForDOA,headers:[{name:"ticket",value:localStorage.getItem('ticket')}] });
    //在初始化的时候 创建模型
    ...
    if (this.applyId) {
      ...
      recordAllowEditStateQuery.FunctionCode = RecordState.reinvoiceReturn;
      recordAllowEditStateQuery.RecordID = this.applyId;
      recordAllowEditStateQuery.NotAllowEditLink = "/reinvoice/return-approve?applyid=" + this.applyId;
...
/**接口请求 begin */
  ...
  getBaseDataSource(){
    this.redApplyService.getBaseDataList().subscribe(
      data => {
        if (data.Result) {
          ...
          if (!this.applyId) {//新建
            //上门取货
            if (this.applyData.apply.homepickup == 0) this.applyData.apply.homepickup = "";
            //公司特殊处理
    ...
    //获取省市县信息
    this.redApplyService.getProvinceCityInfo().subscribe(data => {
      this.provincecityinfo = JSON.parse(data.Data);
    });
  }
  //根据ApplyId获取页面申请数据
    ...
    this.redApplyService.getApplyDataById(applyid).subscribe(
      data => {
        ...
        if (data.Result) {
          ...
          if(subrevisetype && Array.isArray(subrevisetype) && subrevisetype.length > 0){
            if(subrevisetype[0]["subtypecode"] == "801"){
              this.isDOA = true;
            }
          }
          //初始化冲红小类
          ...
          if (this.applyData.apply.revisetypecode == "700") {
            if (tempArray.length > 0) this.childrenRedtypes[0]["checked"] = true;
          }
          if (this.applyData.apply.revisetypecode == "800") {
            if (tempArray.length > 0) {
              this.childrenRedtypes.forEach((item, index, arr) => {
                if(item.revisetypecode == tempArray[0].subtypecode){
                  this.childrenRedtypes[index]["checked"] = "on";
                }else{
                  this.childrenRedtypes[index]["checked"] = "off";
                }
              });
              if (tempArray.length > 0 && tempArray[0]["subtypecode"] == "802") {
                //问题： 如果小类单选情况，如果之前选择的第一个radio，通过on/off初始化选中状态时失败
                //解决方案： 通过jQuery手动添加选中样式
                setTimeout(() => {
                  let radioId = "radio" + this.childrenRedtypes[0]["revisetypecode"];
          ...
          //上门取货
          if (this.applyData.apply.homepickup == -1) this.applyData.apply.homepickup = "";
          //确定冲退明细类型
  ...
  saveApply(applydata) {
    this.loading = true;
    this.applyData.apply.applytype = 1;
    //上门取货
    if (this.applyData.apply.homepickup == "") this.applyData.apply.homepickup = -1;
        ...
        if (data.Result) {
          ...
          if (this.isSubmited) {
            if (this.applyData.apply.wfstatus == "驳回") {
              ...
              this.approveProductReturn(body);
            }else{
  ...
  //流程审批 同意
  approveProductReturn(body) {
    this.redApplyService.approveProductReturn(body).subscribe(
      data => {
        if (data.Result) {
          this.windowService.alert({ message: "提交成功", type: "success" });
  ...
  getInvoiceByOrderNo(orderno) {
    ...
    if(!this.applyData.apply.revisetypecode) {
      this.windowService.alert({ message: "请选择冲退类型", type: "warn" });
      return;
    }
    if (nomber) {
      this.loading = true;
  ...
  getMaterialCallBack(erpmaterialdata: any[]){
    ...
    if(erpmaterialdata && erpmaterialdata.length > 0){
      erpmaterialdata.forEach((internalInvoicenoEle, index) => {
        ...
        //tab文字 如果有外部发票号，则显示外部发票号，如果没有则显示系统发票号
        materialInfo.internalinvoiceno = internalInvoicenoEle["externalinvoiceno"] || internalInvoicenoEle["internalinvoiceno"];
        if (internalInvoicenoEle.material && internalInvoicenoEle.material.length > 0) {
          internalInvoicenoEle.material.forEach((element, index) => {
            ...
            material["num"] = material["originalnum"] = element["num"];
            material["factory"] = element["factory"];
            material["originalstoragelocation"] = material["storagelocation"] = material["specifystoragelocation"] = material["returnstoragelocation"] = element["originalstoragelocation"];
            if (!element["originalbatchno"]) {
              material["originalbatchno"] = material["batchno"] = "";
            }else{
            ...
            material["deliveryno"] = element["deliveryno"];
            if (this.materialType == 2) {
              material["money"] = material["backmoney"] = 0.00;
            }
        ...
        this.applyData.invoice.forEach((invoiceObj, index) => {
          if (invoiceObj.internalinvoiceno == internalInvoicenoEle["internalinvoiceno"]) {
            this.applyData.invoice[index].TransitModeCode = internalInvoicenoEle.material[0]["VSART"];
            let temporaryAddress = new TemporaryAddress();//临时地址信息
            temporaryAddress.active = true;
            ...
            if (internalInvoicenoEle["PRICE_DATE"] === "00000000" || !internalInvoicenoEle["PRICE_DATE"]) {
              this.applyData.invoice[index]["PRICE_DATE"] = null;
            }else{
              let year = String(internalInvoicenoEle["PRICE_DATE"]).substr(0, 4);
              let month = String(internalInvoicenoEle["PRICE_DATE"]).substr(4, 2);
              let day = String(internalInvoicenoEle["PRICE_DATE"]).substr(6, 2);
              this.applyData.invoice[index]["PRICE_DATE"] = year + "-" + month + "-" + day;
            }
            // this.applyData.invoice[index]["PRICE_DATE"] = internalInvoicenoEle["PRICE_DATE"];
            ...
            this.applyData.invoice[index]["SDF_NAME"] = temporaryAddress.SDF_NAME = internalInvoicenoEle["SDF_NAME"];
            this.applyData.invoice[index]["detailaddress"] = temporaryAddress.detailaddress = internalInvoicenoEle["detailaddress"];
            this.applyData.invoice[index]["city"] = temporaryAddress.city = internalInvoicenoEle["city"];
            this.applyData.invoice[index]["zipcode"] = temporaryAddress.zipcode = internalInvoicenoEle["zipcode"];
            this.applyData.invoice[index]["connecter"] = internalInvoicenoEle["connecter"];
            this.applyData.invoice[index]["phone"] = temporaryAddress.phone = internalInvoicenoEle["phone"];
...
/**页面功能 begin */
  ...
  pushInvoice(invoicedata) {
    ...
    invoicedata.forEach((invoiceitem, index) => {
      ...
      if (!hasInvoiceno) {
        ...
        if (tempValid && (applyInvoice.length == 0 || sameCustomer)) {
          ...
          //原发票日期 日期格式处理
          if (this.isBatchUploadForDOA) {
            invoiceObj["invoicedate"] = moment(invoiceitem["invoicedate"]).format("YYYY-MM-DD");
          }else{
            invoiceObj["invoicedate"] = this.getDataYYYYMMDD(invoiceitem["invoicedate"]);
          }
          ...
          if (this.isBatchUploadForDOA) {//批量导入 不需要单独查询基准日及清账号
            invoiceObj["originalreceiptdate"] = invoiceitem["originalreceiptdate"];//收付基准日
            invoiceObj["originalcomplexaccount"] = invoiceitem["originalcomplexaccount"];//清账号
            if (invoiceitem["originalreceiptdate"] === "00000000") {
              invoiceObj["originalreceiptdate"] = null;
            } else {
              invoiceObj["originalreceiptdate"] = moment(invoiceitem["originalreceiptdate"]).format("YYYY-MM-DD");
            }
          }
    ...
    if(!this.isBatchUploadForDOA){//不是批量导入的财务信息
      //4查询首付基准日、清帐号及冲退明细物料
      // 首付基准日、清帐号
  ...
  getChildrenRedtypes(revisetypecode){
    let newArray = [];
    newArray = this.baseDataSource.returntypes.filter(redtype => {
      if (redtype["revisetypecode"] == revisetypecode) return redtype;
    });
    if (newArray.length > 0){
      if (newArray[0]["children"][0]["revisetypecode"] == "801") {
        newArray[0]["children"].reverse();
      }
      this.childrenRedtypes = newArray[0]["children"];
    }
    return newArray;
  }
  ...
  onRevisetypeChange(revisetypecode){
    this.isDOA = false;
    //0、根据id获取大类名称
    let revisetype = this.getNameById(this.baseDataSource.returntypes, "revisetypecode", revisetypecode);
    if (revisetype) this.applyData.apply.revisetype = revisetype["revisetype"];
    //1、根据选中的大类 查找小类数据源
  ...
  onChildrenRevisetypeChange(event, childrenredtype){
    ...
    this.applyData.apply.subrevisetype = "";
    //1、记录需要保存的小类
    ...
    this.confirmMaterialType();
    //4、冲退明细数据处理 
    if (this.applyData.invoice.length > 0 && oldMaterialType != this.materialType) {
      //  后台删除
      this.delDescendantInfo();
      //  前端删除
      this.applyData.invoice.length = this.applyData.material.length = this.materialList.length = 0;
  ...
  applyDataValid(): boolean{
    ...
    if (apply.apply.homepickup === "0" && apply.apply.transittype === "") {
      this.windowService.alert({ message: "请选择运输类型", type: "fail" });
      return false;
    }
    //财务信息是否选择
    if(apply.invoice.length <= 0){
      this.windowService.alert({ message: "请维护财务信息", type: "fail" });
      return false;
    }
    //物料明细必填项
    let tempArr =[];
    this.materialList.forEach(item => {
      let isValid = item.tableList.forEach((item: Material) => {
        item.returnstoragelocation? null: tempArr.push("冲退明细" + item.internalinvoiceno + "有必填项未填写");
      });
    });
    if (tempArr.length > 0) {
    ...
    //审批人信息验证
    if (this.applyData.apply.ApproveUsers["bizDivisionFirst"].length == 0){
  ...
  //批量上传 财务信息
  onBatchUploadInvoice(uploadinvoice: FileUploader){
    this.loading = true;
    this.isBatchUploadForDOA = true;
    this.uploadInvoice.options.additionalParameter = {
      "bizCode":this.selectedBiz["bizcode"],
      "companyCode":this.selectedCompany["companycode"]
    };
    uploadinvoice.queue.map(function (item) {
      item.withCredentials = false;
    });
    uploadinvoice.uploadAll();
    uploadinvoice.onCompleteItem = ((item: FileItem, response: string, status: number, headers: ParsedResponseHeaders) => {
      this.loading = false;
      let data = JSON.parse(response);
      if (status === 200 && data.Result) {
        this.pushInvoice(data.Data);
        this.getMaterialCallBack(data.Data);
        this.isBatchUploadForDOA = false;
      } else {
        this.windowService.alert({ message: data.Message, type: "fail" });
      }
    });
...
/** 冲退明细 begin */
  ...
  deleteMaterial(tabitemi, item, i){
    ...
    if(this.materialList[tabitemi].tableList.length == 1) {this.windowService.alert({ message: "至少保留一行", type: "warn" }); return;}
    //调用后台删除接口
    if (materialid) {
  ...
  //运输方式change事件
  transitModeChange(event,internalinvoiceno){
    let transitMode = this.getNameById(this.baseDataSource.transitmodes, "TransitModeCode", event);
    this.applyData.invoice.forEach((invoiceItem,invoiceItemIndex) => {
      if(internalinvoiceno == invoiceItem.internalinvoiceno && transitMode) {
        this.applyData.invoice[invoiceItemIndex].TransitMode = transitMode.TransitMode;
        this.applyData.invoice[invoiceItemIndex].TransitModeCode = transitMode.TransitModeCode;
      }
    });
  }
  //临时地址change事件
  detailaddressModeChange(event){
    this.temporaryAddressDataSource.forEach(item => { item.active = false; });
    let addressIndex = -1;
    let temporaryaddress = new TemporaryAddress();
    this.temporaryAddressDataSource.forEach((item,index) => {
      if (item.detailaddress === event) addressIndex = index;
    });
    if(addressIndex > -1){
      this.temporaryAddressDataSource[addressIndex].active = true;
      temporaryaddress = this.temporaryAddressDataSource[addressIndex];
    }
    else this.temporaryAddressDataSource[0].active = true;
    //选中项处理
    this.applyData.invoice.forEach((invoiceItem, invoiceIndex) => {
      this.applyData.invoice[invoiceIndex]["SDF_NAME"] = temporaryaddress["SDF_NAME"];
      this.applyData.invoice[invoiceIndex]["detailaddress"] = temporaryaddress["detailaddress"];
      this.applyData.invoice[invoiceIndex]["province"] = temporaryaddress["province"];
      this.applyData.invoice[invoiceIndex]["city"] = temporaryaddress["city"];
      this.applyData.invoice[invoiceIndex]["citycode"] = temporaryaddress["citycode"];
      this.applyData.invoice[invoiceIndex]["district"] = temporaryaddress["district"];
      this.applyData.invoice[invoiceIndex]["zipcode"] = temporaryaddress["zipcode"];
      this.applyData.invoice[invoiceIndex]["connecter"] = temporaryaddress["connecter"];
      this.applyData.invoice[invoiceIndex]["phone"] = temporaryaddress["phone"];
      this.applyData.invoice[invoiceIndex]["signstandard"] = temporaryaddress["signstandard"];
    });
  }
  //添加临时地址
  addTemporaryAddress(){
    this.temporaryAddressModalType = false;
    this.temporaryAddressDataSource.forEach(item => { item.active = false; });
    this.addressModal.show({data:null, provincecityinfo: this.provincecityinfo});//显示弹出框
  }
  //编辑临时地址
  editTemporaryAddress(){
    ...
    this.temporaryAddressDataSource.forEach(item => { item.active = false; });
    this.addressModal.show({data:temporaryaddress[0], provincecityinfo: this.provincecityinfo});//显示弹出框
  }
  //临时地址模态窗关闭时事件
  onReturnAddaddressModalHide = temporaryaddress => {
    if(!temporaryaddress) return;
    //数据源处理
    if(!this.temporaryAddressModalType) this.temporaryAddressDataSource.push(temporaryaddress);
    else this.temporaryAddressDataSource[this.addressIndex] = temporaryaddress;
    //选中项处理
    this.applyData.invoice.forEach((invoiceItem, invoiceIndex) => {
      this.applyData.invoice[invoiceIndex]["SDF_NAME"] = temporaryaddress["SDF_NAME"];
      this.applyData.invoice[invoiceIndex]["detailaddress"] = temporaryaddress["detailaddress"];
  ...
  myToUpCase(tabitemi, i){
    if (this.materialList[tabitemi].tableList[i].returnstoragelocation.length>0 && this.materialList[tabitemi].tableList[i].returnstoragelocation.length < 4) {
      this.materialList[tabitemi].tableList[i].returnstoragelocation = "";
      this.windowService.alert({ message: "库存地格式不对!", type: "fail" });
    }else{
      this.materialList[tabitemi].tableList[i].returnstoragelocation = String(this.materialList[tabitemi].tableList[i].returnstoragelocation).toUpperCase();
    }
    if (this.materialType == 2) {
      if (this.materialList[tabitemi].tableList[i].specifystoragelocation.length>0 && this.materialList[tabitemi].tableList[i].specifystoragelocation.length < 4) {
        this.materialList[tabitemi].tableList[i].specifystoragelocation = "";
        this.windowService.alert({ message: "库存地格式不对!", type: "fail" });
      } else {
        this.materialList[tabitemi].tableList[i].specifystoragelocation = String(this.materialList[tabitemi].tableList[i].specifystoragelocation).toUpperCase();
      }
      this.materialList[tabitemi].tableList[i].batchno = String(this.materialList[tabitemi].tableList[i].batchno).toUpperCase();
    }
  }
  checkReturnNum(tabitemi, i){
    if (Number(this.materialList[tabitemi].tableList[i].returnnum) > Number(this.materialList[tabitemi].tableList[i].originalnum)) {
      this.materialList[tabitemi].tableList[i].returnnum = 0;
      this.windowService.alert({ message: "数量不能大于原单销售数量!", type: "fail" });
    }
  }
  checkNum(tabitemi, i){
    if (Number(this.materialList[tabitemi].tableList[i].num) > Number(this.materialList[tabitemi].tableList[i].originalnum)) {
      this.materialList[tabitemi].tableList[i].num = 0;
      this.windowService.alert({ message: "换货数量不能大于原单销售数量!", type: "fail" });
    }
  }
/** 冲退明细 end */
...
/**审批人信息 begin */
  ...
  isREZTY(){
    ...
    if(this.applyData.apply.platformcode && this.applyData.apply.bizcode){
      ...
      else if (this.applyData.apply.revisetypecode == "800")//换货
      {
        this.isrezty = false;
      }
      //确定是否rezty后 获取审批人信息
